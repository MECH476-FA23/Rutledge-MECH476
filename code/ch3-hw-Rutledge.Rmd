...
print(df_min)
```
    The Highest values all seem to occur around 20:00:00 (evening), and the lowest values all seem to        occur around 7:00:00 (morning.
...
$$\texttt{ozoneugm3} = 1000*\frac{\Big((\texttt{ozone\_ppm})(\text{molecular weight of ozone})(\text{273.15}) (\text{atmospheric pressure}) \Big)}{\Big((\text{universal gas constant})(\text{Celsius to Kelvin value} + \text{temperature})(\text{atmospheric pressure})\Big)}$$
...
mutate(.data = ozone_data, ozoneugm3 = 1000 * (ozone_ppm * molecular_weight_ozone * C_to_K * atm_P_foco) / (R_universal * (C_to_K + temp_foco_c) * atm_P_foco))
```
